...
Bullet::Bullet(AnimationManager manager, double x_coord, double y_coord, double x_speed, bool direction): manager_(manager), x_coord_(x_coord), y_coord_(y_coord), x_speed_(x_speed), direction_(direction) { 
  if (!direction_) {
    x_speed_ *= -1;
  }
}

void Bullet::UpdateBullet(const MapParams& map_params, const std::unordered_set<char>& blocks, double time) {
